...
> **Last verified: 2026-08-01** (web-search-verified against provider docs and independent trackers on this date). Free tiers change frequently — providers add credit-card requirements, cut rate limits, or deprecate models with little notice. Treat every row below as "true as of last verification," not a permanent guarantee. Re-check a provider's own pricing/docs page before depending on it in production.
>
...
**Important finding for this repo:** `backend/src/lib/ai/providers.ts` currently lists `qwen/qwen3-32b` in the Groq model chain — that model's shutdown date (2026-07-17) has already passed as of this doc's verification date. It should be swapped for `qwen/qwen3.6-27b` (or dropped in favor of `openai/gpt-oss-120b`/`openai/gpt-oss-20b`, which are already first/third in that chain). This doc does not modify that `.ts` file — flagging it here per the "generic, drop-in reference" scope of this document; apply the fix in a follow-up code change.
...
This repository (CodeBook e-commerce) ships a working, minimal implementation of exactly this pattern at `backend/src/lib/ai/`:
